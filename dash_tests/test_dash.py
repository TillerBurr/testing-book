...
def test_bsly001_falsy_child(dash_duo):
    ...
    app.layout = html.Div(id="nully-wrapper", children=1)
    ...
    dash_duo.wait_for_text_to_equal("#nully-wrapper", "1", timeout=4)

    # 6. use this form if its present is expected at the action point
    assert dash_duo.find_element("#nully-wrapper").text == "1"
